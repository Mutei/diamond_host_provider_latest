...
class EditMusicServices extends StatefulWidget {
  final bool isVisible;
  final bool allowAdditionalOptions; // New parameter
  final Function(bool, String) onCheckboxChanged;
  final List<String> initialSelectedEntries;

  const EditMusicServices({
    Key? key,
    required this.isVisible,
    required this.allowAdditionalOptions, // Initialize in constructor
    required this.onCheckboxChanged,
    required this.initialSelectedEntries,
  }) : super(key: key);

  @override
  _EditMusicServicesState createState() => _EditMusicServicesState();
}

class _EditMusicServicesState extends State<EditMusicServices> {
  ...
  void _filterOptions() {
    setState(() {
      ...
      if (searchText.isNotEmpty) {
        ...
        // Also filter buffet options if "Is there music" is selected and additional options are allowed
        if (_isMusicSelected() && widget.allowAdditionalOptions) {
    ...
    return Column(
      ...
      children: [
        ...
        */
        ...
        // Show buffet options if "Is there music" is selected and additional options are allowed
        if (_isMusicSelected() && widget.allowAdditionalOptions)
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
  ...
  void _setMainOptionState(String label, bool value) {
    ...
    if (label == 'Is there music' && !value && widget.allowAdditionalOptions) {
      for (var buffet in buffetOptions) {
        if (buffet['value']) {
          buffet['value'] = false;
